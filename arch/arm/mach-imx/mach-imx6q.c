...
static int maxle_prepare_gpios (void)
{
	int uart3_on;
	int res;
	struct device_node *np;

	printk("maxle: prepare gpios\n");
	np = of_find_node_by_path("/soc/aips-bus@02100000/serial@021ec000");
	if (!np)
		return -ENODEV;
	//printk("maxle: found node\n");

	uart3_on = of_get_named_gpio(np, "uart3-on", 0);
	printk("maxle: found gpio  uart3-on as nr=%d\n", uart3_on);

	res = gpio_is_valid(uart3_on);
	//printk("maxle: gpio_is_valid  res=%d\n", res);	
	if (res) {
		res = gpio_request_one(uart3_on, GPIOF_OUT_INIT_LOW | GPIOF_EXPORT_DIR_FIXED, "uart3-on"); 
	    if (res == 0) {	
			gpio_set_value (uart3_on, 0);
			printk("maxle: set 'uart3-on' to 0=inactive\n");		
	    }		
	}
	return 0;
}
...
{
	printk("maxle: mach-imx6q.c - ksz9021rn_phy_fixup()\n");
...
{
	//printk("maxle: mach-imx6q.c  mmd_write_reg device=%d reg=%d val=0x%x\n", device,reg,val);
...
//#define CONGATEC_PATCH
#ifdef CONGATEC_PATCH
struct phy_device *phydev = dev;
	printk("maxle: ---- Congatec ksz9031 (link pulse) patch\n");
...
#else
	printk("maxle: mach-imx6q.c - KWS ksz9031rn_phy_fixup()\n");
	/*
	 * min rx data delay, max rx/tx clock delay,
	 * min rx/tx control delay
	 */
	mmd_write_reg(dev, 2, 4, 0);		/* Control Signal Pad Skew */
	mmd_write_reg(dev, 2, 5, 0);		/* Rx Data Pad Skew */
	mmd_write_reg(dev, 2, 8, 0x003ff);	/* Clock Pad Skew */
#endif
	/* end patch */	
...
//#ifdef OMIT 
	...
	int val;
	val = phy_read(dev, 4);
	printk("maxle: ---- ksz9031 reg4=%04x %04x\n", val, val & 0xf7ff);

	val = phy_read(dev, 9);
#define MASTERMODE_ON
#ifdef MASTERMODE_ON
	phy_write(dev, 9, val | 0x1800);
	printk("maxle: ---- added new ksz9031 MASTER mode selected! %04x %04x\n", val, val | 0x1800);	
#else
        //turn master mode off
  	phy_write(dev, 9, val & ~0x1800);
	printk("maxle: ---- added new ksz9031 MASTER mode OFF! %04x %04x\n", val, val & ~0x1800);	
#endif
    }
//#endif
	...
	printk("maxle: ---- added new ksz9031 congatec patch\n");
...
{
	if (IS_BUILTIN(CONFIG_PHYLIB)) {
		printk("maxle: mach-imx6q.c - register phy's\n");
...
void __init imx6_enet_mac_init(const char *compatible)
{
	...
	{	
	u8 *m;
	if ((m = of_get_mac_address(enet_np))!=NULL)
	{
		printk("maxle: mach-imx6q.c - of_get_mac_address %x:%x:%x:%x:%x:%x\n",
	...
	printk("maxle: mach-imx6q.c - OCOTP_MAC %x:%x:%x:%x:%x:%x\n",
...
static void __init imx6q_init_machine(void)
{
	...
	maxle_prepare_gpios(); // maxle added
	imx6q_enet_init();
	imx_anatop_init();
	imx6_pm_init();
	imx6q_csi_mux_init();
