...
static inline int evervision_parser_packet(u8 *buff, struct evervision_data *evervision)
{
    ...
    switch (EVENT) {
        ...
        case 2:
            ...
            break;
        case 1:
            input_report_abs(evervision->input, ABS_X, X );   // wiwi
            input_report_abs(evervision->input, ABS_Y, Y );   // wiwi
...
static int evervision_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_debug("%s: irq = %d \n", __func__, client->irq);
	printk("wiwi: evervision_ts_probe() 1\n");
    ...
    if (pdata == NULL) {
 	...
 	printk("wiwi: evervision_ts_probe() 2\n");   
	...
	printk("wiwi: evervision_ts_probe() 3\n");
	...
	pdata->input->evbit[0] = BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS); //wiwi
	pdata->input->keybit[BIT_WORD(BTN_TOUCH)] = BIT_MASK(BTN_TOUCH); //wiwi


   	max_x = 2048; // wiwi  pdata->x_range;
	max_y = 2048; // wiwi  pdata->y_range;
    ...
    //input_set_abs_params(pdata->input, ABS_PRESSURE, 0, 10, 0, 0);  wiwi
